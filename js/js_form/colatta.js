$(document).ready(function(){
	...
	$('#btn_save').click(function(){
		...
		$("#form_editing").ajaxSubmit({
			url:site_url("colatta/add"),
			type: 'post',
			success: function(){

				alert('Colatta Product Succesfully Added.', {title: 'Added Sukses.',titleClass: 'info', modal: true, buttons: [{id: 0, label:'OK', val: 'X'}],callback:function(){ window.setTimeout('window.location="<?php echo base_url();?>colatta"',500); }});
				window.location = base_url + 'colatta';

				$('#prod_colatta').DataTable().destroy();
				$('#prod_colatta tbody').html('');
	...
	$('#btn_edit').click(function(){
		...
		$("#form_editing").ajaxSubmit({
			url:site_url("colatta/edit"),
			type: 'post',
			success: function(){

				alert('Colatta Product Succesfully Update.', {title: 'Added Sukses.',titleClass: 'info', modal: true, buttons: [{id: 0, label:'OK', val: 'X'}],callback:function(){ window.setTimeout('window.location="<?php echo base_url();?>colatta"',500); }});
				window.location = base_url + 'colatta';
...
function modul_add(){
	...
	$('#namefile').val('');
	$('#desc').val('');
	$('#judul').text("Add Product Colatta");
	$('#modal_colatta').modal('show');
	$('#save').show();
	$('#edit').hide();
}

function get_colatta(){
	...
	$.ajax({
		...
		type:"POST",
		url:base_url+"colatta/get_colatta",
		dataType:"json",
		// data:param,
		success:function(data){
			...
			if(content_data!=''){
				$('#prod_colatta tbody').html(content_data);
				$('#prod_colatta').DataTable({
					"paging":   false,
					"ordering": false,
					"info":     false,
					"searching": false
...
function edit(e){
	...
	$('#judul').text("Edit Product Colatta");
	$('#modal_colatta').modal('show');
	$('#edit').show();
	$('#save').hide();
	...
	bootbox.confirm("Are You Sure Want Delete This Data ?", function(result) {
		...
		if(result==true){
			...
			$.ajax({
				...
				type:"POST",
				url:base_url+"colatta/delete",
				dataType:"html",
				data:str_param,
				success:function(){

					alert('Colatta Product Succesfully Delete.');
